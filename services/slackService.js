...
ioEmitter.on('message_rules_processed', async (processedMessage) => {
  if (processedMessage.platform === 'slack') {
    try {
      ...
      if (saved) {
        ioEmitter.emit('message_updated', saved.toObject());
      }
    } catch (err) {
      console.error('Error updating processed message:', err);
    }
  }
});
...
export async function processSlackEvent(team_id, event) {
  if (event.type === 'message' && event.channel && event.user && event.text) {
    ...
    if(userId) {
      // Emit event for rules processing instead of direct call
      ioEmitter.emit('process_message_rules', { message: msg, userId });
    }
